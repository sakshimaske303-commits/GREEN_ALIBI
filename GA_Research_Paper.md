...
- SIF's decline came before NDVI's in 7 of 8 growing seasons studied.
- Drought years did not show a bigger SIF-NDVI lag than normal years.
- A cross-correlation check found NDVI leading SIF in 3 of 8 years.
- SIF-rainfall correlation held up, but weaker than an earlier estimate.
- Satellite signals beat the 2018 official drought declaration by 7-8 weeks.

## Abstract

Drought detection in India, for official declarations and PMFBY (Pradhan Mantri Fasal Bima Yojana) crop-insurance payouts, still leans mainly on rainfall records and the Normalized Difference Vegetation Index (NDVI), which only respond once a crop has already been damaged. This study tests whether Solar-Induced Fluorescence (SIF), a satellite signal tied to photosynthesis, detects that stress earlier than NDVI. I used GOSIF v2 SIF data and cloud-screened MODIS NDVI across the 8 districts of Marathwada, Maharashtra, over 8 growing seasons (2015-2023, excluding 2021), checked against a 20-year CHIRPS rainfall climatology. Only 2 of those 8 years, 2015 (-21.5%) and 2018 (-18.3%), met this study's own drought threshold. Under a threshold-crossing method, SIF's decline came before NDVI's in 7 of the 8 years; 2018 was a near-flat exception at -1.1 days. Drought years did not show a bigger lag than normal years (7.6 vs 15.0 days), so H3 is not supported. A separate cross-correlation check gave a more mixed picture: SIF led in 4 years, tied in 1 (2020), and NDVI led in 3 years (2018, 2022, 2023), with 2022 and 2023 backed by a bootstrap where over 99% of replicates fell below zero. At the district level, SIF and rainfall anomaly correlated significantly (r = 0.567, p < 0.0001, n = 64 district-years). Both SIF and NDVI crossed their stress thresholds roughly 7 to 8 weeks before Maharashtra's official 2018 drought declaration. SIF shows a real, physically grounded lead over NDVI in most years, but not every year, and the bigger opportunity for faster drought relief lies in satellite monitoring generally arriving well ahead of the current declaration process.
...
**Figure 1. [AI Generated]** Schematic representation of leaf-level light energy partitioning among photochemistry, non-photochemical quenching (heat dissipation), and chlorophyll fluorescence, under well-watered versus drought-stressed conditions. Diagram generated using ChatGPT (GPT-5, July 2026) from a prompt describing this concept; checked for scientific accuracy by the author.
...
**Figure 2. [AI Generated]** Leaf cross-section illustrating red-light absorption by chlorophyll in the palisade mesophyll versus near-infrared scattering at spongy mesophyll air-cell interfaces, the physical basis of the reflectance contrast measured by NDVI. Diagram generated using ChatGPT (GPT-5, July 2026) from a prompt describing this concept; checked for scientific accuracy by the author.
...
The 95% confidence intervals: 2015, 4 days [0, 11]; 2016, 2 days [−3, 7]; 2017, 27 days [0, 30]; 2018, −4 days [−13, 3]; 2019, 18 days [−9, 26]; 2020, 0 days [−32, 32]; 2022, −9 days [−16, −2]; 2023, −10 days [−25, −2]. 2022 and 2023's intervals sit entirely below zero. So these two years are genuinely different from a zero lag, this is not just one negative-looking number by chance. 2020's interval is enormous, [−32, 32], reflecting how little this particular year's sparse observations pin down a direction at all.
...
- District-level rainfall anomaly was computed using one region-wide climatological baseline. A separate baseline for each district was not built.
- Section 4.9 compares SIF- and NDVI-based stress timing against the one well-documented official drought-declaration date I could find (Maharashtra, 31 October 2018). No comparably verifiable declaration date turned up for any other study year, so this is only an example from a single year. It should not be read as a general finding across all eight — and as Section 4.9 details, even this single year's SIF-versus-NDVI ordering flipped after the Section 3.2 boundary correction, a reminder not to treat this comparison as more stable or precise than its sample size can actually support.
