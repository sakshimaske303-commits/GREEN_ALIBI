...
Solar-Induced Fluorescence (SIF) is a satellite signal that comes from the plant's own photosynthesis process, not how it looks from outside. Photosynthetic efficiency changes before a crop's green color does, so SIF should pick up drought stress earlier than NDVI. This study tested that idea directly, using GOSIF v2 and cloud-screened MODIS NDVI over Marathwada's 8 districts, across 9 growing seasons (2015–2023), and checked it independently against CHIRPS rainfall data. 2 of these 9 years (2015, 2018) meet this study's own rainfall-anomaly drought threshold; the other 7 are normal-to-wet monsoon years.

## What Was Found

SIF leads NDVI in most years, but not all of them. In 8 of the 9 years studied, SIF's seasonal decline started before NDVI's, under the main method used. A second, different check (cross-correlation) agrees for most years but not all. A bug in the original code turned up here too: it had been written to only ever look in 1 direction, so it could never have shown NDVI leading SIF even if the data actually said so. Once fixed, the corrected check shows SIF leading in 5 years, basically tied in 1, and NDVI leading in 3 (2018, 2022, 2023). 2018 is an exception under every method used: NDVI actually crosses the key stress threshold slightly *before* SIF that year. This is reported directly, not smoothed over, and the code bug itself is written out in full in `GA_Development_Log.md` (Entry 17), not quietly patched and left unmentioned.
...
Drought severity does not simply make the lag bigger. The natural idea here was that a more severe drought would produce a bigger SIF-NDVI gap. This was tested directly, at nearly triple the original sample size, and it did not hold up: the 2 drought years averaged a smaller lag (7.6 days) than the 7 normal years (15.1 days). This negative result is reported directly, not adjusted or hidden.

The stress pattern lines up with rainfall on its own, though more weakly than an earlier, smaller sample had suggested. District-level SIF stress and rainfall deficit are significantly correlated (Pearson r = 0.531, p < 0.001, across 72 district-year observations), and this was also checked for spatial artifacts using a Moran's I diagnostic. Rainfall's spatial clustering shows up consistently every year (significant in all 9 years), while SIF's spatial clustering only shows up in fewer than half the years (4 of 9). This is a real, mixed result, not a uniform one.

It also lines up with actual harvests, not just other satellite or rainfall data. Official government crop-yield statistics for the same districts show the 2 drought years harvesting measurably less than the normal years (Yield Anomaly Index −1.08 vs. +0.07), the same direction the satellite and rainfall record already point — not yet statistically significant given only 2 drought years to compare, but a real, external, ground-outcome check that this study's drought classification isn't just an artifact of comparing one satellite product to another.
...
This is a 9-year proof-of-concept, not a working operational system. It has been checked against official government crop-yield statistics (see above), but not against a field-level crop-stress survey; it covers just 1 region; the 9 years are still unbalanced toward normal conditions (2 drought years versus 7 normal); 2018's exception to the main finding isn't explained yet by anything checked so far; and GOSIF (the SIF product used) is itself partly modeled from MODIS data, so it isn't fully separate from the NDVI it's being compared against. These limits, and a few more, are reported in full in `GA_Research_Paper.md`.
